...
- Version change: 3.0.0 → 3.1.0 (MINOR - Refines Phase III specifications for Hackathon II alignment)
...
- Templates requiring updates:
  - ✅ spec-template.md: Already includes MCP tool sections
  - ✅ plan-template.md: Already includes AI/Agent sections
  - ✅ tasks-template.md: Already includes Agent behavior categories
- Follow-up TODOs:
...
**Timeline**: Phase I (Dec 7) → Phase II (Dec 14) → Phase III (Dec 21) → Phase IV (Jan 4) → Phase V (Jan 18)
...
|-------|--------|-------------|--------|-----------|
| **Phase I** | ✅ **COMPLETE** | In-Memory Python Console App | 100 | Dec 6, 2025 |
| **Phase II** | ✅ **COMPLETE** | Full-Stack Web Application (Next.js + FastAPI + Neon + Better Auth) | 150 | Dec 13, 2025 |
| **Phase III** | 🚧 **PLANNED** | AI-Powered Todo Chatbot (OpenAI Agents SDK + MCP) | 200 | In Progress |
| **Phase IV** | 📋 **PLANNED** | Local Kubernetes Deployment (Minikube + Helm) | 250 | Not Started |
| **Phase V** | 📋 **PLANNED** | Advanced Cloud Deployment (DOKS + Kafka + Dapr) | 300 | Not Started |
...
- MCP tool JSON schemas (parameters, return types, error cases) - EXACTLY 5 tools:
  ...
  2. `list_tasks` - user_id, status (all/pending/completed) → array of tasks
...
│       └── phase-3-chatbot/
│           ├── spec.md
│           ├── agent-spec.md       # OpenAI Agents SDK specs
...
│   │   ├── components/
│   │   │   ├── TaskList.tsx
│   │   │   └── ChatBot.tsx  # Phase III - ChatKit wrapper (to be created)
│   │   ├── lib/
│   │   │   ├── api.ts
│   │   │   └── chatApi.ts   # Phase III - Chat endpoint client (to be created)
│   │   └── types/
│   │       └── conversation.ts  # Phase III types (to be created)
│   └── CLAUDE.md
...
│   │   │   ├── tasks.py
│   │   │   ├── auth.py
│   │   │   └── chat.py      # Phase III - POST /api/{user_id}/chat (to be created)
│   │   ├── models/
│   │   │   ├── task.py
│   │   │   ├── user.py
│   │   │   └── conversation.py  # Phase III - Conversation & Message models (to be created)
│   │   ├── services/
│   │   │   ├── task_service.py
│   │   │   ├── agent_service.py  # Phase III - OpenAI Agents SDK wrapper (to be created)
│   │   │   └── mcp_service.py    # Phase III - MCP tool definitions (to be created)
│   │   ├── auth/
│   │   └── db/
│   │       └── migrations/
│   │           └── versions/
│   │               └── 003_add_conversation_tables.py  # Phase III migration (to be created)
│   └── CLAUDE.md
...
```
User Input (ChatKit) → POST /api/{user_id}/chat (FastAPI)
    ↓
...
- ❌ BAD: "Traceback: IndexError on line 42"
- ✅ GOOD: "I couldn't find that task"

- ❌ BAD: "Foreign key constraint violation"
- ✅ GOOD: "Task not found. Try 'Show me my tasks'"

- ❌ BAD: "Connection timeout to Neon"
- ✅ GOOD: "I'm having trouble accessing your tasks. Please try again in a moment"
...
- ✅ Spec acceptance criteria met (all 40 tasks marked [X])
- ✅ Performance budgets met (chat response < 3s p95)
- ✅ Agent behavior validated (intent recognition, tool usage)
- ✅ MCP tools validated (5 tools working correctly)
